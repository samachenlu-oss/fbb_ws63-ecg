...
static errcode_t sle_uart_server_add(void)
{
    ...
    ret = SsapsRegisterServer(&app_uuid, &g_serverId);
    if (ret != ERRCODE_SLE_SUCCESS) {
        printf("%s SsapsRegisterServer fail, ret:%x\r\n", SLE_UART_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
...
errcode_t sle_uart_server_init()
{
    ...
    if (!enable_ret) {
        printf("%s EnableSle returned false, wait callback\r\n", SLE_UART_SERVER_LOG);
    }
    printf("%s init ok\r\n", SLE_UART_SERVER_LOG);
    return ERRCODE_SLE_SUCCESS;
}
